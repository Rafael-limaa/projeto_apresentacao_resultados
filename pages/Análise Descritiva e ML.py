...
st.markdown("### Nossos dados são compostos por essas características")
...
st.write("### A seguir, apresentamos a análise da distribuição dos dados com base nas seguintes variáveis: gênero, formação dos pais, qualidade da internet e qualidade da dieta.")
...
with col6:
   st.write(
    """
    A análise do boxplot revela dois pontos principais:
    - **Outlier Identificado:** A categoria '2' apresenta um valor atípico (outlier), com uma nota significativamente distinta das demais.
    - **Tendência Positiva:** Observa-se uma clara tendência de que notas mais altas estão associadas a uma melhor avaliação da saúde mental.
    """)
   fig2, ax = plt.subplots(figsize=(8, 6))
   sns.boxplot(data=filtered_df, x='Avaliação de Saúde Mental', y='Nota da Prova', palette='Set2', ax=ax)
   st.pyplot(fig2)
...
st.write(f"R-squared (R2):** {r2 * 100:.2f}% - Proporção da variabilidade da nota que o modelo consegue explicar.")

st.write(f"## O modelo aplicado explica {r2 * 100:.2f}% variabilidade das notas.")
